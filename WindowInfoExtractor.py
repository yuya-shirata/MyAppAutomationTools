...
取得する情報には、ウィンドウのタイトル、クラス名、コントロールタイプ、位置とサイズ、PIDが含まれます。
"""
...
def print_window_info(win, file, indent=0):
    ...
    if "Edit" in control_type_info:
        ...
        try:
            selected_info = "Selected: " + str(win.get_toggle_state() == 1)
            print(indent_str + selected_info)
            file.write(indent_str + selected_info + "\n")
        except Exception as e:
            error_info = "Error getting toggle state: " + str(e)
            print(indent_str + error_info)
            file.write(indent_str + error_info + "\n")
...
# このプログラムは、デスクトップ上のすべてのウィンドウの情報を取得し、
# それをテキストファイルに出力します。特定のコントロールの情報も取得されます。
desktop = Desktop(backend="uia")
 
# ファイルを書き込みモードで開く
with open("WindowInfoOutput.txt", "w", encoding="utf-8") as file:
